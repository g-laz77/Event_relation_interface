...
def make_window():
    ...
    w2 = OptionMenu(frame2, var, "BEFORE", "AFTER", "INCLUDES", "IS_INCLUDED", "DURING", "DURING_INV", "SIMULTANEOUS",
...
def add_entry():
    ...
    if var2.get() == "LINK":
        fpoint.write("\n<LINK lid=\"" + str(
            a) + "\" relType=\"" + variable.get() + "\" eventID=\"" + sour_evt.get() + "\" relatedToEvent=\"" + dest_evt.get() + "\">")
        a += 1
    else:
        fpoint.write("\n<TLINK lid=\"" + str(
            bb) + "\" relType=\"" + var1.get() + "\" eventID=\"" + sour_evt.get() + "\" relatedToTime=\"" + dest_tim.get() + "\">")
        b += 1
    fpoint.close()
    set_select()
...
def set_select():
    ...
    # select5.tag_configure("highlight", background="yellow")
    # add the tag "highlight" to the characters between index 3.0 and 4.0
    # select5.tag_add("highlight", "3.0", "4.0")
